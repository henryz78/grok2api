...
可以直接使用仓库内的 Dockerfile 部署到 Railway。`railway.toml` 已配置端口
`8000` 和 `/healthz` 健康检查。即使没有挂载 `config.yaml`，服务也能使用默认
配置启动，部署完成后进入管理页面设置即可。

为了持久保存账号和设置，请把 Railway Volume 挂载到 `/app/data`。基础启动
不要求额外环境变量；如需自动初始化或显式管理密钥，可设置：
...
未设置 `GROK2API_PUBLIC_API_BASE_URL` 时会自动使用 Railway 提供的公网域名。
产生数据后，请保持凭据加密密钥不变。
